...
- **THEN** o objeto SHALL conter: `id`, `ementa`, `sigla_tipo`, `numero`, `ano`, `data_apresentacao`, `descricao_situacao`, `status` (alias), `sigla_partido`, `partido`, `categorias`, `subtemas` (array com o nome de todas as categorias vinculadas, `[]` se nenhuma), `classificacao_status`
