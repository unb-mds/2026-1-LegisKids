...
- [x] Push feito e PR aberto para `develop` — `backend-ci` rodou de verdade contra o Postgres do GitHub Actions e **encontrou um bug real e pré-existente nas migrations**, não relacionado a esta mudança: a migration `ded4438e4a2c` tentava `DROP CONSTRAINT favoritos_usuario_id_proposicao_id_key` (nome autogerado do Postgres, só existente em bancos com histórico anterior à criação da migration base) e recriar um índice/constraint que a migration base (`babbb73a5d52`) já cria com o nome canônico. Nunca foi pego antes porque o time só roda migrations contra o Neon de produção, que já está em `head` (upgrade vira no-op) — o CI foi a primeira execução de `flask db upgrade` contra um banco genuinamente vazio.
- [x] **Corrigido**: `migrations/versions/ded4438e4a2c_auditoria_backend_total_descartados_e_.py`, etapa 3 (favoritos) — tornada idempotente com o mesmo padrão condicional (`inspector.get_unique_constraints`) já usado na etapa 2 do próprio arquivo para a coluna `categoria` legada. `drop_constraint` só roda se o nome antigo existir; `create_index` ganhou `if_not_exists=True`; `create_unique_constraint` só roda se o nome canônico ainda não existir. Verificado que as outras duas migrations (`ca57241159f9`, `f3a1b2c4d5e6`) não têm o mesmo problema (não mexem em constraints/índices). Validado localmente: `flask db upgrade` continua no-op limpo contra o Neon (já em head) — a simulação de banco vazio via schema Postgres isolado esbarrou numa limitação do pooler do Neon (`unsupported startup parameter: search_path`) e foi abortada sem alterar nada compartilhado; a correção final será validada de fato no próximo push ao PR.
